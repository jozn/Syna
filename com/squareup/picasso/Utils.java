...
final class Utils {
    ...
    static int calculateMemoryCacheSize(Context context) {
        ...
        int memoryClass = (((context.getApplicationInfo().flags & 1048576) != 0 ? 1 : null) == null || VERSION.SDK_INT < 11) ? activityManager.getMemoryClass() : ActivityManagerHoneycomb.getLargeMemoryClass(activityManager);
    ...
    static String createKey(Request request, StringBuilder stringBuilder) {
        if (request.stableKey != null) {
            stringBuilder.ensureCapacity(request.stableKey.length() + KEY_PADDING);
            stringBuilder.append(request.stableKey);
        } else if (request.uri != null) {
            String uri = request.uri.toString();
            stringBuilder.ensureCapacity(uri.length() + KEY_PADDING);
            stringBuilder.append(uri);
        } else {
            stringBuilder.ensureCapacity(KEY_PADDING);
        ...
        if (request.centerCrop) {
            stringBuilder.append("centerCrop").append(KEY_SEPARATOR);
        } else if (request.centerInside) {
            stringBuilder.append("centerInside").append(KEY_SEPARATOR);
        }
    ...
    static int getBitmapBytes(Bitmap bitmap) {
        int byteCount = VERSION.SDK_INT >= WEBP_FILE_HEADER_SIZE ? BitmapHoneycombMR1.getByteCount(bitmap) : bitmap.getRowBytes() * bitmap.getHeight();
        if (byteCount >= 0) {
            return byteCount;
        }
    ...
    static boolean isWebPFile(InputStream inputStream) throws IOException {
        byte[] bArr = new byte[WEBP_FILE_HEADER_SIZE];
        return inputStream.read(bArr, 0, WEBP_FILE_HEADER_SIZE) == WEBP_FILE_HEADER_SIZE && WEBP_FILE_HEADER_RIFF.equals(new String(bArr, 0, 4, StringUtils.USASCII)) && WEBP_FILE_HEADER_WEBP.equals(new String(bArr, 8, 4, StringUtils.USASCII));
    }
