...
public class AndroidVideoApi9JniWrapper {
    ...
    final class C02961 implements PreviewCallback {
    ...
    private static int[] findClosestEnclosingFpsRange(int i, List<int[]> list) {
        Log.m6026d("Searching for closest fps range from " + i);
        ...
        for (int[] iArr3 : list) {
            if (iArr3[0] <= i && iArr3[1] >= i) {
                ...
                if (abs2 < abs) {
                    Log.m6026d("a better range has been found: w=" + iArr3[0] + ",h=" + iArr3[1]);
                    iArr4 = iArr3;
                    i2 = abs2;
                } else {
                    i2 = abs;
        ...
        Log.m6026d("The closest fps range is w=" + iArr2[0] + ",h=" + iArr2[1]);
        return iArr2;
    }

    public static int[] selectNearestResolutionAvailable(int i, int i2, int i3) {
        Log.m6026d("selectNearestResolutionAvailable: " + i + ", " + i2 + DataForm.ELEMENT + i3);
    ...
    private static void setCameraDisplayOrientation(int i, int i2, Camera camera) {
        ...
        Log.m6034w("Camera preview orientation: " + (cameraInfo.facing == 1 ? (360 - ((cameraInfo.orientation + i) % 360)) % 360 : ((cameraInfo.orientation - i) + 360) % 360));
        try {
            camera.setDisplayOrientation(cameraInfo.facing == 1 ? (360 - ((cameraInfo.orientation + i) % 360)) % 360 : ((cameraInfo.orientation - i) + 360) % 360);
        } catch (Exception e) {
            Log.m6028e("Failed to execute: camera[" + camera + "].setDisplayOrientation(" + r0 + ")");
            e.printStackTrace();
        }
    }
    ...
    public static Object startRecording(int i, int i2, int i3, int i4, int i5, long j) {
        Log.m6026d("startRecording(" + i + ", " + i2 + ", " + i3 + ", " + i4 + ", " + i5 + ", " + j + ")");
        try {
            ...
            open.addCallbackBuffer(new byte[bitsPerPixel]);
            open.setPreviewCallbackWithBuffer(new C02961(j));
            setCameraDisplayOrientation(i5, i, open);
            open.startPreview();
            AndroidVideoApi5JniWrapper.isRecording = true;
            Log.m6026d("Returning camera object: " + open);
            return open;
        } catch (Exception e) {
            e.printStackTrace();
            return null;
