...
class AndroidCameraConfigurationReader5 {
    ...
    public static AndroidCamera[] probeCameras() {
        ...
        if (Hacks.isGalaxySOrTab()) {
            Log.m6026d("Hack Galaxy S : has one or more cameras");
            if (Hacks.isGalaxySOrTabWithFrontCamera()) {
                Log.m6026d("Hack Galaxy S : HAS a front camera with id=2");
                arrayList.add(new AndroidCamera(2, true, 90, supportedPreviewSizes));
            } else {
                Log.m6026d("Hack Galaxy S : NO front camera");
            }
            Log.m6026d("Hack Galaxy S : HAS a rear camera with id=1");
            ...
            if (Hacks.hasTwoCamerasRear0Front1()) {
                Log.m6026d("Hack SPHD700 has 2 cameras a rear with id=0 and a front with id=1");
