...
public class AndroidVideoApi5JniWrapper {
    ...
    final class C02941 implements PreviewCallback {
    ...
    public static void activateAutoFocus(Object obj) {
        Log.m6026d("mediastreamer", "Turning on autofocus on camera " + obj);
    ...
    protected static void applyCameraParameters(Camera camera, int i, int i2, int i3) {
        ...
        if (supportedPreviewFrameRates != null) {
            ...
            Log.m6026d("mediastreamer", "Preview framerate set:" + parameters.getPreviewFrameRate());
        }
        camera.setParameters(parameters);
    }

    public static int detectCameras(int[] iArr, int[] iArr2, int[] iArr3) {
        Log.m6026d("detectCameras\n");
        int i = 0;
        for (AndroidCamera androidCamera : AndroidCameraConfiguration.retrieveCameras()) {
            if (i == 2) {
                Log.m6034w("Returning only the 2 first cameras (increase buffer size to retrieve all)");
                break;
            }
    ...
    public static int[] selectNearestResolutionAvailable(int i, int i2, int i3) {
        Log.m6026d("mediastreamer", "selectNearestResolutionAvailable: " + i + ", " + i2 + DataForm.ELEMENT + i3);
    ...
    protected static int[] selectNearestResolutionAvailableForCamera(int i, int i2, int i3) {
        ...
        if (list == null) {
            Log.m6028e("mediastreamer", "Failed to retrieve supported resolutions.");
            return null;
        }
        Log.m6026d("mediastreamer", list.size() + " supported resolutions :");
        for (Size size : list) {
            Log.m6026d("mediastreamer", "\t" + size.width + DataForm.ELEMENT + size.height);
        }
        ...
        try {
            ...
            Log.m6026d("mediastreamer", "resolution selection done (" + new int[]{size2.width, size2.height, i8}[0] + ", " + new int[]{size2.width, size2.height, i8}[1] + ", " + new int[]{size2.width, size2.height, i8}[2] + ")");
            return new int[]{size2.width, size2.height, i8};
        } catch (Exception e) {
            Log.m6028e("mediastreamer", "resolution selection failed");
            e.printStackTrace();
            return null;
        }
    }
    ...
    public static void setPreviewDisplaySurface(Object obj, Object obj2) {
        Log.m6026d("mediastreamer", "setPreviewDisplaySurface(" + obj + ", " + obj2 + ")");
        try {
    ...
    public static Object startRecording(int i, int i2, int i3, int i4, int i5, long j) {
        Log.m6026d("mediastreamer", "startRecording(" + i + ", " + i2 + ", " + i3 + ", " + i4 + ", " + i5 + ", " + j + ")");
        Camera open = Camera.open();
        applyCameraParameters(open, i2, i3, i4);
        open.setPreviewCallback(new C02941(j));
        open.startPreview();
        isRecording = true;
        Log.m6026d("mediastreamer", "Returning camera object: " + open);
        return open;
    }

    public static void stopRecording(Object obj) {
        isRecording = false;
        Log.m6026d("mediastreamer", "stopRecording(" + obj + ")");
        Camera camera = (Camera) obj;
        if (camera != null) {
        ...
        Log.m6032i("mediastreamer", "Cannot stop recording ('camera' is null)");
    }
}
